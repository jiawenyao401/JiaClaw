...
- ✅ **可选 glob** - 默认注册的工作区按模式找文件工具（`pattern` 必填）
...
- ✅ **Optional glob** - registered by default (`pattern` required)
  - Optional `path` (relative search root, default `.`), `max_results` (default 100, clamped to 1..=500)
